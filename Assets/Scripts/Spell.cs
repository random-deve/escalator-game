...
public class Spell : MonoBehaviour
{
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        {
            if (continuousDamage && other.GetComponent<Enemy>())
            {
                other.GetComponent<Enemy>().TakeDamage(stayDamage * Time.deltaTime);
            }
        }
    }
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag(damageTag) && timer <= dieAfter - startDamageAfter && (stopDamageAfter == 0 || timer >= dieAfter - stopDamageAfter))
        {
            ...
            {
                other.GetComponent<Enemy>().TakeDamage(hitDamage);
            }
        }
    }
    ...
    private void OnCollisionEnter(Collision collision)
    {
        if (collision.gameObject.CompareTag(damageTag) && collisionDamage)
        {
            ...
            {
                collision.gameObject.GetComponent<Enemy>().TakeDamage(hitDamage);
            }
        }
    }
}
